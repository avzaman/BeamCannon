...
struct CachedForgery {
    std::vector<uint8_t> pkt_template; // The original genuine MAC headers
    std::vector<uint8_t> forged_feedback; // The malicious matrix
    int feedback_len = 0;
    int fb_off = 0;
    bool ready = false;
};
...
private:
    std::map<std::string, CachedForgery> forgery_cache_;
    std::string iface_;
    APInfo      ap_;
    Logger&     logger_;
    AttackStats stats_;
    ...
    std::string build_bpf(const std::vector<ClientInfo>& targets,
                                  const ClientInfo* quartermaster);
